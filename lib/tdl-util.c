...
FILE* tdl_create(const char* path, const char* name)
{
	...
	char buff[1024];
	// Add .tdl to the path if it was missing from the path provided
	// by the caller.
	if(pathLen < 4 || strncmp(path + pathLen - 4, ".tdl", 4) != 0)
	{
		int retVal = snprintf(buff, 1024, "%s.tdl", path);
		if(retVal >= 1024)
			msg(MSG_WARNING, "Path provided to tdl_create() was too long: %s", path);
		path = buff;
		pathLen += 4;
	}
	...
	if(f == NULL)
	{
		msg(MSG_ERROR, "Failed to create file %s", path);
		return f;
	}
	...
	{
		msg(MSG_ERROR, "Failed to write header to %s", path);
		return f;
	}
	...
	{
		msg(MSG_ERROR, "Failed to write object name to %s", path);
		return f;
	}
