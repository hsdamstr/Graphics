...
typedef struct {
	...
	int hasData; /**< Has data been written to? Set by handle_tracker() callback. */
...
static void smooth(TrackedObject *to)
{
	...
	//printf(" pre kalman quat: %f %f %f %f\n", to->data.quat[0], to->data.quat[1], to->data.quat[2], to->data.quat[3] );
	...
	//printf("post kalman quat: %f %f %f %f\n", to->data.quat[0], to->data.quat[1], to->data.quat[2], to->data.quat[3]);
...
static void VRPN_CALLBACK handle_tracker(void *name, vrpn_TRACKERCB t)
{
	...
	/* Create a place to store position and orientation locally. */
	float pos[3], quat[4];
	...
	/* Some tracking systems return exceptionally large positions when the tracked point is lost. 
	   In this case, don't update our record. */
...
int vrpn_is_vicon(const char *hostname)
{
	...
	if(strstr(hostname, "169.254.") != NULL) // Hard-wired vicon IP address
		return 1;
...
/** Given an VRPN object name and a (possibily NULL) hostname,
    generate a object\@hostname string. The config file value
    specified in "vrpn.server" will be consulted if the hostname
    argument is NULL.
 ...
 * "vrpn.server" value specified in a config file.
 *
...
/** Gets a set of records from VRPN before they are processed. This is
    ...
    hostname specified in a config file with the "vrpn.server" key.
