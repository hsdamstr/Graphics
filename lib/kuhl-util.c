...
/* Variables to make it easier to loop over all of the different types of textures assimp handles. */
...
/* We make STB available even if imagemagick is present */
...
void kuhl_diagnostics(void)
{
	...
	for(int i=0; i<numMonitors; i++) // for each monitor
	{
		...
		// TODO: Add glfwGetMonitorContentScale() here once GLFW 3.3
		// is commonplace. glfw 3.2 is on Ubuntu 20.04.
...
static GLFWwindow* kuhl_glfw_create_window(int width, int height, const char *title)
{
	...
	{
		// TODO: Maybe do something with glfwGetWindowContentScale()
		// here once GLFW 3.3 is commonplace. glfw 3.2 is on Ubuntu
		// 20.04.
...
/* Assigns app-icon.png image to be the window's icon. */
static void kuhl_set_window_icon(GLFWwindow *window)
{
	stbi_set_flip_vertically_on_load(0);

	int width=-1, height=-1, comp=-1;
	int requestedComponents = STBI_rgb_alpha;
	char *filename = kuhl_find_file("../images/app-icon.png");
	...
	{
		msg(MSG_DEBUG, "Failed to load app icon %s\n", filename);
	}
	free(image);
...
void kuhl_geometry_attrib(kuhl_geometry *geom, const GLfloat *data, GLuint components, const char* name, int warnIfAttribMissing)
{
	...
	{
		msg(MSG_WARNING, "Unable to add attribute '%s' to the geometry object. You requested %d components but it must be 1, 2, 3, or 4.\n", name, components);
		return;
	}
	if(!glIsVertexArray(geom->vao))
	{
...
/** Draws a kuhl_geometry object. Typically, instances == 1. If
 * instances > 1, the object will be drawn multiple times with
 * different 'gl_InstanceID' variables in the GLSL program.
 *
 ...
 *
 * @param Number of instanced objects to draw. To draw an object once
 * normally (i.e., non-instanced), set it to 1.
 */
void kuhl_geometry_draw_instanced(kuhl_geometry *geom, GLsizei instances)
{
	if(geom == NULL)
		return;
	if(geom->vertex_count == 0)
	{
		msg(MSG_WARNING, "You tried to draw geometry which contained 0 vertices.");
		return;
	}
	...
	if(instances < 1)
	{
		msg(MSG_WARNING, "You tried to draw less than 1 instance of an object.");
		return;
	}
	...
	else
	{
		...
		if(instances == 1)
			...
			glDrawArraysInstanced(geom->primitive_type, 0, geom->vertex_count, instances);
		kuhl_errorcheck();
	}
...
{
	kuhl_geometry_draw_instanced(geom, 1);
}
...
void kuhl_geometry_delete(kuhl_geometry *geom)
{
	...
	// Delete any other geometry objects in the list too---but
...
/* Make a label using imagemagick. See kuhl_make_label() for more info. */
float kuhl_make_label_im(const char *label, GLuint *texName, float color[3], float bgcolor[4], float pointsize)
{
#ifdef KUHL_UTIL_USE_IMAGEMAGICK
	int width = 0;
	int height = 0;
...
float kuhl_make_label_8x8(const char *label, GLuint *texName, float color[3], float bgcolor[4], float pointsize)
{
	int len = strlen(label);
	int width = 8*len;  // width and height of output image in pixels
	int height = 8;
	unsigned char *image = malloc(width*height*4);

	for(int c=0; c<len; c++) // for each character
	{
		// Index into the image array for the lower left corner of the
		// character we are on.
		int ll = 8*4*c;
		for(int x=0; x<8; x++) // for each row
		{
			for(int y=0; y<8; y++) // for each column
			{
				// Index in image for this pixel
				int start = ll+(y*width+x)*4;
				
				// Check that character is ascii
				unsigned int charInt = (unsigned int) label[c];
				if(charInt > 127) // font8x8_basic has only ascii characters
					charInt = 63; // use '?' character

				// Get 8 bits representing this row.
				char bits = font8x8_basic[charInt][7-y];
				if(bits >> x & 1)
				{
					image[start+0] = color[0]*255;
					image[start+1] = color[1]*255;
					image[start+2] = color[2]*255;
					image[start+3] = 255;
				}
				else
				{
					image[start+0] = bgcolor[0]*255;
					image[start+1] = bgcolor[1]*255;
					image[start+2] = bgcolor[2]*255;
					image[start+3] = bgcolor[3]*255;
				}
			}
		}
	}
	...
	*texName = kuhl_read_texture_rgba_array(image, width, height);
	...
	// Texture defaults to bilinear filter for magnification which
	// makes this low resolution texture look very blurry. Change this
	// to nearest neighbor so we get 'chunky' appearance for the text.
...
/** Creates a texture from a string of text. For example, it can
 * create a texture that says "hello world" in red on a trasnparent
 * background.
 *
 * This function uses a hardcoded 8x8 monospace bitmap font. The pointsize
 * argument may actually be ignored.
 *
 * Previously, we used to have ImageMagick render text at much higher
 * quality. However, ImageMagick is not required for opengl-examples
 * and it is nice to have code that always works regardless of if
 * ImageMagick is present or not. Also, ImageMagick 6 and 7 have minor
 * differences that may or may not be fixe in imageio.c.
 * 
 *
 ...
 *
 * @param pointsize The size of the text in points. Use a larger value
 * for higher resolution text. This feature only works if imagemagick
 * is used to make the label.
 *
 * @return The aspect ratio of the texture. Upon error, texName
 * is set to 0 and an aspect ratio of 1 is returned. */
float kuhl_make_label(const char *label, GLuint *texName, float color[3], float bgcolor[4], float pointsize)
{
	// See comment above kuhl_make_label() for why we use 8x8 bitmap 
	// instead of ImageMagick code.
	return kuhl_make_label_8x8(label, texName, color, bgcolor, pointsize);

	//return kuhl_make_label_im(label, texName, color, bgcolor, pointsize);
}
...
static float kuhl_read_texture_file_im(const char *filename, GLuint *texName, GLuint wrapS, GLuint wrapT)
{
	...
	if(image == NULL)
	{
		msg(MSG_ERROR, "Unable to read '%s' with ImageMagick.\n", filename);
		return -1;
	}
	...
	*texName = kuhl_read_texture_array(image, width, height, 4, wrapS, wrapT);
	...
	if(*texName == 0)
	{
		msg(MSG_ERROR, "Failed to create OpenGL texture from %s (image was read with ImageMagick).\n", filename);
		return -1;
	}
...
#endif  // end ifdef KUHL_UTIL_USE_IMAGEMAGICK
...
static float kuhl_read_texture_file_stb(const char *filename, GLuint *texName, GLuint wrapS, GLuint wrapT)
{
	...
	if(image == NULL)
	{
		msg(MSG_ERROR, "Unable to read '%s' with STB.\n", filename);
		return -1;
	}
	...
	*texName = kuhl_read_texture_array(image, width, height, 4, wrapS, wrapT);
	...
	if(*texName == 0)
	{
		msg(MSG_ERROR, "Failed to create OpenGL texture from %s (image was read with STB).\n", filename);
		return -1;
	}
...
/** Uses either STB (preferred) or ImageMagick (a fallback) to read an
...
float kuhl_read_texture_file_wrap(const char *filename, GLuint *texName, GLuint wrapS, GLuint wrapT)
{
	...
	/* First try loading with STB. It supports most common image
	 * formats and since it is in our codebase, are more certain it
	 * will behave the same across machines. */
	float aspectRatio = kuhl_read_texture_file_stb(filename, texName, wrapS, wrapT);
#ifdef KUHL_UTIL_USE_IMAGEMAGICK
	/* If imagemagick is present, try using it if STB failed. */
	if(aspectRatio < 0)
	{
		msg(MSG_WARNING, "STB failed to load texture '%s'. Retrying with ImageMagick...", filename);
		aspectRatio = kuhl_read_texture_file_im(filename, texName, wrapS, wrapT);
	}
#endif
	return aspectRatio;
}
...
static void kuhl_screenshot_stb(const char *outputImageFilename)
{
	...
	else if(strlen(s) > 4 && !strcmp(s + strlen(s) - 4, ".jpg"))
		ok = stbi_write_jpg(s, windowWidth, windowHeight, comp, data, 95);
...
/** Takes a screenshot of the current OpenGL screen and writes it to an image file.
    ...
    @param outputImageFilename The name of the image file that you want to record the screenshot in. The type of image file is determined by the filename extension. Suggestion: PNG files often work best for screenshots; try "output.png".
*/
...
/** The texture paths in model files are often broken. Here, we assume
    that the texture path stored in the model is relative to the
    directory the model is stored in. Or, if textureDir is provided,
    we assume that the texture is relative to the textureDir path.

    @param textureFile is the path to the texture stored in the model
    file.
...
static char* kuhl_private_assimp_fullpath(const char *textureFile, const char *modelFile, const char *textureDir)
{
	...
	if(textureDir == NULL)
	{
		...
		/* Here, make the full path:
		   modelFileDirectoryName + textureFileFilename
		*/
		char *editableModel = strdup(modelFile);
		char *editableTexture = strdup(textureFile);
#ifdef _WIN32
		char model_drive[32];
		char model_dir[1024];
		char model_fname[128];
		char model_ext[8];
		_splitpath_s(editableModel, model_drive, 32, model_dir, 1024, model_fname, 128, model_ext, 8);

		char texture_drive[32];
		char texture_dir[1024];
		char texture_fname[128];
		char texture_ext[8];
		_splitpath_s(editableTexture, texture_drive, 32, texture_dir, 1024, texture_fname, 128, texture_ext, 8);

		snprintf(fullpath, 1024, "%s%s%s%s", model_drive, model_dir, texture_fname, texture_ext);
#else
		/* The texture path is in the model file, so if the model was
		 * generated on windows, there may be \ instead of / in the
		 * paths. */
		for(unsigned int i=0; i<strlen(editableTexture); i++)
			if(editableTexture[i] == '\\')
				editableTexture[i] = '/';
		char *model_dname = dirname(editableModel);
		char *texture_bname = basename(editableTexture);
		snprintf(fullpath, 1024, "%s/%s", model_dname, texture_bname);
#endif
		free(editableTexture);
		free(editableModel);
...
static const struct aiScene* kuhl_private_assimp_load(const char *modelFilename, const char *textureDirname)
{
	...
	//	msg(MSG_DEBUG, "%s: This model has %u texture(s) embedded in it.\n",
	...
	{
		for(int tt=0; tt<TEX_TYPE_LEN; tt++) // for each texture type
		{
			// how many textures are there of this type?
			int textureCount = aiGetMaterialTextureCount(scene->mMaterials[m], texTypeList[tt]);
			if(textureCount == 0)
				continue;

			/* What kinds of textures should we try to load? ASSIMP
			 * supports many more than these three types (see
			 * texTypeList array). But, we'll just load these three
			 * for now. */
			if(texTypeList[tt] != aiTextureType_DIFFUSE &&
			   texTypeList[tt] != aiTextureType_SPECULAR &&
			   texTypeList[tt] != aiTextureType_NORMALS)
			{
				msg(MSG_DEBUG, "Ignoring %s texture(s) in model.", texTypeListStr[tt]);
				continue;
			}
			else
			{
				msg(MSG_DEBUG, "Processing %s texture type\n", texTypeListStr[tt]);
			}
			
			if(textureCount > 1)
				msg(MSG_DEBUG, "There are multiple %s textures in the model. Ignoring all except the first one.", texTypeListStr[tt]);

			struct aiString path;
			GLuint texIndex = 0;
			if(aiGetMaterialTexture(scene->mMaterials[m], texTypeList[tt],  texIndex, &path, NULL, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS)
			{
				...
				/* Load the embedded texture. In this case,
				   path.data will be *1, *2, etc...
				   fullpath may have "./" prepended before "*1"
				*/
				if(path.data[0] == '*')
				{
					int embeddedInt = atoi(&path.data[1]);
					struct aiTexture *embTex = scene->mTextures[embeddedInt];
					unsigned char *image = NULL;
					
					msg(MSG_INFO, "Loading embedded texture %s width width %d, height %d, hint %s\n", modelFilename, embTex->mWidth, embTex->mHeight, embTex->achFormatHint);
					if(embTex->mHeight == 0) // if compressed, mHeight=0 and mWidth is length of array
					{
						int components;
						int width, height;
						image = stbi_load_from_memory((unsigned char *)embTex->pcData, embTex->mWidth, &width, &height, &components, 4);
						//FILE *f = fopen("/tmp/img.jpg", "w");
						//fwrite(embTex->pcData, embTex->mWidth, 1, f);
						//fclose(f);
						//exit(1);
						
						msg(MSG_WARNING, "Compressed image actually has width %d, height %d, components %d\n", width, height, components);
						texIndex = kuhl_read_texture_array(image, width, height, 4, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
						free(image);
					}
					else
					{
						msg(MSG_FATAL, "Loading uncompressed texture embedded in model file: %s\n NOT SUPPORTED.", fullpath);
						exit(1);
					}
				}
				/* Or, read the external texture */
				else if(kuhl_read_texture_file(fullpath, &texIndex) < 0)
				...
				msg(MSG_WARNING, "Failed to load a %s texture for an unknown reason.\n", texTypeListStr[tt]);
			
		} // end for each texture type loop.

#if 0
		// count the number of unhandled textures.
		int textureCount = 0;
		for(int i=0; i<TEX_TYPE_LEN; i++)
		{
			if(strcmp(texTypeListStr[i] == "DIFFUSE") == 0) // skip diffuse
				continue;
                                    ...
                                    unsigned int animationNum, double t)
{
	...
	//	return 0;
	/* OR, if the time value is too large, use the transformation at the final tick. */
	if(currentTick > anim->mDuration)
		currentTick = anim->mDuration;
 ...
 */
void kuhl_bbox_fit(float result[16], const float bbox[6], const int sitOnXZPlane)
{
...
/** Edits the GeomTransform matrix in the geometry object so that the
 * object will fit in a 1x1x1 box. It will be placed at the location
 * x,y,z. It will be centered at that location or sit on "top" of that
 * location if sitOnXZPlane is non-zero. */
void kuhl_make_geom_fit(kuhl_geometry *geom, const float bbox[6], const int sitOnXZPlane, const int x, const int y, const int z)
{
	if(geom == NULL || bbox == NULL)
	{
		msg(MSG_WARNING, "A parameter passed to kuhl_make_geom_fit() was NULL");
		return;
	}

	/* Get a matrix that scales makes the geometry fit in a 1x1x1 box
	 * either centered at the origin or on the origin. */
	float fitMat[16];
	kuhl_bbox_fit(fitMat, bbox, sitOnXZPlane);

	/* Translate the model from the origin to the requested location. */
	float transMat[16];
	mat4f_translate_new(transMat, x, y, z);

	// fitmat = transmat * fitmat
	mat4f_mult_mat4f_new(fitMat, transMat, fitMat);

	kuhl_geometry *geomFirst = geom;
	// Iterate through all geometry objects in list
	do {
		// Store a copy of fit matrix so we can reuse it if kuhl_update_model() is called later.
		mat4f_copy(geom->fitMatrix, fitMat);

		mat4f_mult_mat4f_new(geom->matrix, fitMat, geom->matrix);
		geom = geom->next;
	} while(geom != NULL);

	/* If the model has bones, normally the "fit" matrix is gets
	 * applied in kuhl_load_model(). However, it is possible that the
	 * developer never calls it. It does get called once in
	 * kuhl_load_model(), but that is before someone might call this
	 * function. So, we call kuhl_update_model() again here. */
	kuhl_update_model(geomFirst,0,-1);
}
...
GLint kuhl_gen_framebuffer(int width, int height, GLuint *texture, GLuint *depthTexture)
{
	...
	if(texture != NULL)
	{
		...
		glBindTexture(GL_TEXTURE_2D, *texture);
// if color.linear is set, shouldn't we make the framebuffer use srgb too?
...
// if color.linear is set, shouldn't we set this to SRGB too?
//		glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, colorSamples, GL_SRGB,
