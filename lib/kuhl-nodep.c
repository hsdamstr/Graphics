...
{
	msg(MSG_DEBUG, "Trying to read file %s", filename);
...
/** Given a filename, tries to find that file by:
   ...
   5) Remove the absolute path so we only use the filename.

   6) Remove the absolute path and just use the filename with the
   extra paths to check in the commonDirs list.
...
char* kuhl_find_file(const char *filename)
{
	...
	strncpy(commonDirs[commonDirsLen++], "../images", 255);
	...
	*/
	...
	/* Get the filename (ignoring the path before it) */
	const char *basenameFilename = filename + strlen(filename);
	while(basenameFilename > filename)
	{
		if(*basenameFilename == '/' || *basenameFilename == '\\')
		{
			basenameFilename += 1;
			break;
		}
	...
	if(kuhl_can_read_file(basenameFilename))
	{
		free(pathSepChange);
		return strdup(basenameFilename);
	}
	
	for(int i=0; i<commonDirsLen; i++)
	{
		newPath = kuhl_path_concat_read(commonDirs[i], basenameFilename);
		if(newPath)
		{
			free(pathSepChange);
			return newPath;
