...
void main() 
{
	...
	else if(renderStyle == 4)
	{
		...
		vec3 camLook = normalize(-out_Position_CC);
		float diffuse = clamp(dot(camLook, normalize(out_Normal_CC)), 0,1) / 2+.5;
	...
	{
		/* out_Position_CC is the position of this fragment in camera
		 * coordinates. Since the camera is at 0,0,0 in camera
		 * coordinates, we can interpret it as a vector pointing from
		 * the camera to the fragment. out_Normal_CC is a normal vector
		 ...
		 * directions. */
		if(dot(out_Normal_CC, out_Position_CC) < 0)
			fragColor = vec4(0,.3,0,1); // green are front faces
		else
			fragColor = vec4(.3,0,0,1); // red are back faces
	}
